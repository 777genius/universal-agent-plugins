...
export function useDirectoryStatus(maintainClock = false) {
  ...
  if (maintainClock) {
    const refreshClock = () => {
      ...
      const untilExpiry = registry.expires_at
        ? Date.parse(registry.expires_at) - now.value + 1
        : 60_000;
  ...
  return {
    ...
    current: computed(
      () =>
        registry.data_source === 'published_snapshot' && !directoryIsExpired(registry, now.value),
    ),
  };
}
