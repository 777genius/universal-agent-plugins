...
const EVIDENCE_WORKFLOW =
  /^[a-z0-9][a-z0-9-]*\/[a-z0-9][a-z0-9._-]*\/\.github\/workflows\/[A-Za-z0-9._-]+\.ya?ml$/;
...
function stringArray(value: unknown, field: string, context: string): string[] {
  ...
  if (new Set(values).size !== values.length)
    throw new Error(`${context}: ${field} must be unique`);
  return values;
}
...
function parseRFC3339Instant(value: unknown, context: string): ParsedInstant {
  ...
  if (zone !== 'Z') {
    ...
    if (offsetHour > 23 || offsetMinute > 59)
      throw new Error(`${context} has an invalid RFC3339 offset`);
...
function source(value: unknown, context: string, allowUnresolved = false): PluginSource {
  ...
  const revision =
    value.revision === null && allowUnresolved
      ? null
      : requiredString(value, 'revision', `${context} source`);
  ...
  if (revision !== null && !REVISION.test(revision))
    throw new Error(`${context}: source revision must be a full commit SHA`);
  return {
    repository,
    revision,
    path: requiredString(value, 'path', `${context} source`),
    manifest_sha256: digestValue(
      value.manifest_sha256 ?? value.manifest_digest,
      `${context} source manifest digest`,
    ),
    tree_sha256: digestValue(
      value.tree_sha256 ?? value.tree_digest,
      `${context} source tree digest`,
    ),
    ...(value.icon_sha256 === undefined
      ? {}
      : { icon_sha256: digestValue(value.icon_sha256, `${context} source icon digest`) }),
  };
}
...
function icon(value: unknown, context: string): PluginIcon {
  ...
  return {
    path: requiredString(value, 'path', `${context} icon`),
    sha256: digestValue(value.sha256, `${context} icon digest`),
  };
}

function clientIDs(value: unknown, context: string): ClientID[] {
  const values = stringArray(value, 'clients', context);
  if (values.some((client) => !CLIENTS.has(client as ClientID)))
    throw new Error(`${context}: contains an invalid client`);
...
function legacyAuthentication(name: string): RegistryPlugin['authentication'] {
  if (['agent-code-navigator', 'chrome-devtools', 'cloudflare-docs', 'context7'].includes(name))
    return 'none';
...
function legacyTargetAuthentication(
  authentication: RegistryPlugin['authentication'],
): TargetAuthentication {
...
function legacyEvidence(value: unknown, context: string): ClientEvidence[] {
  ...
  if (value.schema !== 'agent-plugins-1.0')
    throw new Error(`${context}: validation schema is invalid`);
...
function parseLegacyIndex(input: Record<string, unknown>): RegistryIndex {
  ...
  const plugins = input.plugins.map((raw, index): RegistryPlugin => {
    ...
    if (typeof raw.built_in !== 'boolean')
      throw new Error(`${context}: built_in must be a boolean`);
    ...
    if (raw.built_in ? installSource !== name : installSource !== expectedSource) {
      throw new Error(
        `${context}: ${raw.built_in ? 'built-in install_source must equal its name' : 'external install_source must use source repository@40-char-sha//path'}`,
      );
    }
    if (!record(raw.client_support))
      throw new Error(`${context}: client_support must be an object`);
    const resolution = raw.client_support.resolution;
    if (resolution !== 'catalog' && resolution !== 'install_time')
      throw new Error(`${context}: client support resolution is invalid`);
    if (
      (raw.built_in && resolution !== 'catalog') ||
      (!raw.built_in && resolution !== 'install_time')
    ) {
    ...
    if (!compatibleClients.length)
      throw new Error(`${context}: at least one compatible client is required`);
    ...
    if (components.some((component) => !COMPONENTS.has(component)))
      throw new Error(`${context}: unsupported component`);
    ...
    const distribution: DistributionView = {
      ...
      selectable: true,
      targets: compatibleClients.map((client) => ({
        client,
        authentication: legacyTargetAuthentication(authentication),
        delivery: legacyDelivery[client]!,
        scopes: ['user'],
      })),
      components,
      releases: [],
...
function releaseTargets(value: unknown, context: string): ReleaseTarget[] {
  ...
  return value.map((raw, index): ReleaseTarget => {
    ...
    if (!CLIENTS.has(client) || seen.has(client))
      throw new Error(`${context}: target ${index} has an invalid or duplicate client`);
    seen.add(client);
    const delivery = requiredString(raw, 'delivery', `${context} target ${client}`);
    if (!['managed', 'prepared', 'manual_activation'].includes(delivery))
      throw new Error(`${context}: target ${client} has invalid delivery`);
    const authentication = requiredString(
      raw,
      'authentication',
      `${context} target ${client}`,
    ) as TargetAuthentication;
    if (!['not_required', 'required', 'unknown'].includes(authentication))
      throw new Error(`${context}: target ${client} has invalid authentication`);
    ...
    if (record(raw.app_binding)) {
      appBinding = {
        app_key: requiredString(
          raw.app_binding,
          'app_key',
          `${context} target ${client} app binding`,
        ),
        id: requiredString(raw.app_binding, 'id', `${context} target ${client} app binding`),
        mcp_server: requiredString(
          raw.app_binding,
          'mcp_server',
          `${context} target ${client} app binding`,
        ),
    ...
    if (client === 'chatgpt' && !appBinding)
      throw new Error(`${context}: ChatGPT target requires app_binding`);
    if (client !== 'chatgpt' && appBinding)
      throw new Error(`${context}: app_binding is valid only for ChatGPT`);
    return {
      client,
      authentication,
      delivery: delivery as ReleaseTarget['delivery'],
      scopes,
      ...(appBinding ? { app_binding: appBinding } : {}),
    };
  });
}

function evidenceTrustedForEligibility(
  item: Record<string, unknown>,
  level: EvidenceLevel,
  signerVouched: boolean,
): boolean {
  ...
  if (trust.kind === 'github_actions') {
    ...
    if (
      !workflow ||
      !EVIDENCE_WORKFLOW.test(workflow) ||
      !sourceRef ||
      !EVIDENCE_SOURCE_REF.test(sourceRef) ||
      !sourceDigest ||
      !REVISION.test(sourceDigest) ||
      !record(artifact) ||
      workflow.slice(0, workflow.indexOf('/.github/workflows/')) !== artifact.repository ||
      artifact.revision !== sourceDigest
    )
      return false;
    return true;
  }
...
function evidenceFromSnapshot(
  ...
  signerVouched: boolean,
): { client: ClientEvidence[]; package: PackageEvidence[] } {
  ...
  for (const item of input) {
    ...
    if (item.distribution_id !== distributionID || item.release_sequence !== releaseSequence)
      continue;
    if (item.package_tree_digest !== treeDigest) continue;
    if (!selectedIDs.includes(String(item.id))) continue;
    const level = item.level;
    const outcome = item.outcome;
    if (!['schema', 'materialization', 'discovery', 'runtime', 'oauth'].includes(String(level)))
      continue;
    if (
      !['passed', 'failed', 'inconclusive', 'not_tested', 'not_applicable'].includes(
        String(outcome),
      )
    )
      continue;
    if (
      ...
      typeof item.artifact.repository !== 'string' ||
      !REPOSITORY.test(item.artifact.repository) ||
      typeof item.artifact.revision !== 'string' ||
      !REVISION.test(item.artifact.revision) ||
      typeof item.artifact.path !== 'string' ||
      !item.artifact.path ||
      typeof item.artifact.digest !== 'string' ||
      !DIGEST.test(item.artifact.digest)
    )
      continue;
    const artifact = {
    ...
    const common = {
      ...
      trusted_for_eligibility: evidenceTrustedForEligibility(
        item,
        level as EvidenceLevel,
        signerVouched,
      ),
      ...(optionalString(item, 'observed_at')
        ? { tested_at: optionalString(item, 'observed_at') }
        : {}),
      artifact,
    };
    if (level === 'schema') {
    ...
    client.push({
      ...
      architecture,
      tested_at: testedAt,
      ...(optionalString(item, 'dependency_identity')
        ? { dependency_identity: optionalString(item, 'dependency_identity') }
        : {}),
    });
  }
...
function parseSnapshot(
  ...
  mode: 'published_snapshot' | 'review_preview',
): RegistryIndex {
  const isSigned = input.snapshot_schema_version === 1;
  if (
    (!isSigned && input.schema_version !== 1) ||
    !Array.isArray(input.products) ||
    !Array.isArray(input.distributions)
  ) {
  ...
  if (
    ...
    (!isSigned || typeof input.generated_at !== 'string' || typeof input.expires_at !== 'string')
  ) {
    throw new Error(
      'published snapshot requires one signed sequence, generated_at, and expires_at',
    );
  }
  if (isSigned) safeSequence(snapshotSequence, 'Directory snapshot sequence');
  else if (snapshotSequence !== undefined)
    safeSequence(snapshotSequence, 'Directory preview snapshot sequence');
  ...
  if (input.generated_at !== undefined || input.expires_at !== undefined || isSigned) {
    ...
    if (compareInstants(expires, generated) <= 0)
      throw new Error('Directory expires_at must be after generated_at');
  ...
  const revoked = new Set(
    (Array.isArray(input.revocations) ? input.revocations : [])
      .filter(record)
      .map((item) => `${String(item.distribution_id)}:${String(item.release_sequence)}`),
  );
  ...
  const plugins = input.products.map((raw, index): RegistryPlugin => {
    ...
    if (!PLUGIN_NAME.test(name) || seen.has(name))
      throw new Error(`${context}: invalid or duplicate id ${name}`);
    seen.add(name);
    ...
    if (!listed.includes(defaultID))
      throw new Error(`${context}: default distribution is not listed`);
    if (!record(raw.minimum_capabilities))
      throw new Error(`${context}: minimum_capabilities must be an object`);
    const minimumCapabilities = raw.minimum_capabilities;
    const requiredComponents = new Set<ComponentID>(
      [...COMPONENTS].filter((component) => minimumCapabilities[component] === 'required'),
    );
    const distributions = listed.map((id): DistributionView => {
      const item = distributionRecords.get(id);
      if (!item || item.product_id !== name)
        throw new Error(`${context}: missing or mismatched distribution ${id}`);
      ...
      if (!Array.isArray(item.releases) || !item.releases.length)
        throw new Error(`distribution ${id}: releases are required`);
      const status = requiredString(item, 'status', `distribution ${id}`);
      if (!['candidate', 'active', 'suspended'].includes(status))
        throw new Error(`distribution ${id}: unsupported status`);
      const policies = Array.isArray(item.release_policies)
        ? item.release_policies.filter(record)
        : [];
      for (const [policyIndex, policy] of policies.entries()) {
        safeSequence(
          policy.release_sequence,
          `distribution ${id} policy ${policyIndex} release sequence`,
        );
      }
      ...
      if (new Set(releaseSequences).size !== releaseSequences.length)
        throw new Error(`distribution ${id}: release sequences must be unique`);
      const releaseViews = releases.map((release): DistributionReleaseView => {
        const releaseSequence = safeSequence(
          release.sequence,
        ...
        if (!policy)
          throw new Error(
            `distribution ${id}: release ${String(releaseSequence)} has no signed policy`,
          );
        const targets = releaseTargets(
          policy.targets,
          `distribution ${id} release ${String(releaseSequence)}`,
        );
        const policyStatus = requiredString(policy, 'status', `distribution ${id} release policy`);
        if (!['active', 'superseded', 'revoked'].includes(policyStatus))
          throw new Error(`distribution ${id}: unsupported release status`);
        const releaseStatus = revoked.has(`${id}:${String(releaseSequence)}`)
          ? 'revoked'
          : (policyStatus as DistributionView['release_status']);
        if (
          !Array.isArray(policy.current_evidence) ||
          policy.current_evidence.some((value) => typeof value !== 'string')
        )
          throw new Error(`distribution ${id}: current_evidence must be an array of evidence IDs`);
        const treeDigest = digestValue(release.tree_digest, `distribution ${id} tree digest`);
        const evidence = evidenceFromSnapshot(
          input.evidence ?? input.verification_summaries ?? input.current_verification,
          id,
          releaseSequence,
          treeDigest,
          policy.current_evidence as string[],
          mode === 'published_snapshot',
        );
        const components = stringArray(
          release.components ?? [],
          'components',
          `distribution ${id}`,
        ) as ComponentID[];
        if (components.some((component) => !COMPONENTS.has(component)))
          throw new Error(`distribution ${id}: unsupported component`);
        const blockingClients = [
          ...new Set(
            evidence.client
              .filter(
                (observation) =>
                  observation.trusted_for_eligibility &&
                  observation.outcome === 'failed' &&
                  ['materialization', 'discovery', 'runtime'].includes(observation.level),
              )
              .map((observation) => observation.client),
          ),
        ];
        const materializedClients = [
          ...new Set(
            evidence.client
              .filter(
                (observation) =>
                  observation.trusted_for_eligibility &&
                  observation.level === 'materialization' &&
                  observation.outcome === 'passed',
              )
              .map((observation) => observation.client),
          ),
        ];
        const meetsMinimumCapabilities = [...requiredComponents].every((component) =>
        ...
        return {
          ...
          version:
            optionalString(release, 'version') ??
            optionalString(release, 'package_version') ??
            'unversioned',
          targets,
          components,
      ...
      const eligibleTargets = (release: DistributionReleaseView) =>
        release.selectable
          ? release.targets.filter(
              (target) =>
                !release.blocking_clients.includes(target.client) &&
                (kind !== 'upstream' || release.materialized_clients.includes(target.client)),
            )
          : [];
      const selectedRelease =
        releaseViews.find((release) => eligibleTargets(release).length > 0) ?? releaseViews[0]!;
      const compatible = [
        ...new Set(
          releaseViews.flatMap((release) =>
            eligibleTargets(release).map((target) => target.client),
          ),
        ),
      ];
      return {
    ...
    const selected = declared.selectable
      ? declared
      : (distributions
          .filter((item) => item.selectable)
          .sort((a, b) => priority[a.kind] - priority[b.kind] || a.id.localeCompare(b.id))[0] ??
        declared);
    const components = selected.components;
    const productAuthor = record(raw.author)
      ? author(raw.author, context)
    ...
    return {
      ...
      ...(raw.icon === undefined
        ...
        : {
            icon: icon(
              record(raw.icon) && raw.icon.sha256 === undefined
                ? { path: raw.icon.path, sha256: raw.icon.digest }
                : raw.icon,
              context,
            ),
          }),
        ...
        : {
            default_fallback_reason:
              declared.status !== 'active'
                ? `Declared default is ${declared.status}`
                : `Declared default release is ${declared.release_status}`,
          }),
      distributions,
      ...
      client_support: {
        ...
        clients: [
          ...new Set(
            distributions
              .filter((item) => item.selectable)
              .flatMap((item) => item.compatible_clients),
          ),
        ],
        delivery: Object.fromEntries(
          selected.targets.map((target) => [target.client, target.delivery]),
        ),
        scopes: Object.fromEntries(
          selected.targets.map((target) => [target.client, target.scopes]),
        ),
        app_bindings: Object.fromEntries(
          selected.targets
            .filter((target) => target.app_binding)
            .map((target) => [target.client, target.app_binding!]),
        ),
      },
    };
  });
...
export function parseDirectoryData(
  ...
  if ('plugins' in input) {
    if (mode === 'published_snapshot')
      throw new Error(
        'published snapshot mode requires signed snapshot products and distributions',
      );
...
export function evidenceLabel(value: ClientEvidence | PackageEvidence): string {
  const level =
    value.level === 'oauth' ? 'OAuth' : value.level[0]!.toUpperCase() + value.level.slice(1);
...
export function validationLabel(
  view: Pick<RegistryPlugin, 'evidence' | 'package_evidence'>,
): string {
  const passed = view.evidence.filter((item) => item.outcome === 'passed');
  const hasEnvironment = (item: ClientEvidence) =>
    Boolean(item.client_version && item.os && item.architecture && item.tested_at);
  if (passed.some((item) => item.level === 'oauth' && hasEnvironment(item))) return 'OAuth tested';
  if (passed.some((item) => item.level === 'runtime' && hasEnvironment(item)))
    return 'Runtime tested';
  if (passed.some((item) => item.level === 'discovery' && hasEnvironment(item)))
    return 'Discovery tested';
  if (passed.some((item) => item.level === 'materialization' && hasEnvironment(item)))
    return 'Materialization tested';
    ...
    if (release.release_status !== 'active') {
      ...
      if (distribution.kind === 'upstream') {
        const missing = [...targets]
          .filter((target) => !release.materialized_clients.includes(target))
          .sort();
        if (missing.length) {
          reasons.push(
...
export function resolveDistribution(
  ...
  const resolved = (distribution: DistributionView): DistributionView | undefined => {
    ...
    return release
      ? {
          ...
          compatible_clients: release.targets
            .filter((target) => !release.blocking_clients.includes(target.client))
  ...
  const declared = plugin.distributions.find(
    (distribution) => distribution.id === plugin.declared_default_distribution,
  );
  ...
  const alternatives = plugin.distributions
    .filter((item) => item.id !== declared.id)
  ...
  for (const distribution of alternatives) {
    ...
    ineligibleReasons.push({
      ...
      reason: eligibleRelease(distribution, selectedTargets).reason ?? 'no releases',
    });
  }
...
export function expectedDistribution(
  plugin: RegistryPlugin,
  targets: readonly ClientID[],
): DistributionView | undefined {
  return resolveDistribution(plugin, targets).distribution;
}

export function directoryIsExpired(
  registry: Pick<RegistryIndex, 'data_source' | 'expires_at'>,
  now = Date.now(),
): boolean {
...
export function githubSourceUrl(
  plugin: RegistryPlugin,
  distribution = defaultDistribution(plugin),
): string {
  if (distribution.source.revision === null)
    return `https://github.com/${distribution.source.repository}`;
