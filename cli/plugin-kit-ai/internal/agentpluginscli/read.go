...
func checkManagedIntegrity(ctx context.Context, app App, client domain.DetectedClient, installation domain.Installation, binding domain.ClientBinding) []doctorFinding {
	...
	target, err := (clientplanner.Planner{ManagedRoot: app.ManagedRoot, Paths: pathpolicy.Policy{}, Registry: app.ClientRegistry}).ResolveTarget(ctx, client, domain.InstallScope(binding.Scope), physicalID)
