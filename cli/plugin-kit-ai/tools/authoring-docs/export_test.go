...
func TestDeterministicSurfacesAndNoActions(t *testing.T) {
	...
	for i, s := range m.Surfaces {
		...
		if i == 0 {
			...
			for _, n := range []string{"bootstrap", "import", "import help", "import native", "normalize"} {
				want[s.CommandPath+" "+n] = true
			}
		}
...
func TestExportBytesAndLinks(t *testing.T) {
	...
	if len(m.Surfaces) != 2 || len(m.Surfaces[0].Commands) != 21 || len(m.Surfaces[1].Commands) != 20 || m.FactoryBaseline != factoryBaselineSHA || m.SourceSHA != sha || m.Released || m.Status != "prepared-not-release" {
