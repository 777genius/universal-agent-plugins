...
- Codex has no in-place plugin slot. `plugin add` copies to cache; list `source.path` is managed source; `plugin marketplace upgrade` on a local marketplace reports empty success (`upgradedRoots: []`). Do not call it.
...
Agent Plugins Claude user-scope delivery is the official **in-place `@skills-dir` plugin slot**: `ConfigRoot/skills/<physicalArtifactID>`, identity `DeclaredName@skills-dir` with `installPath` the same directory as `ActivePath` (`os.SameFile`, not spelling) and `scope=user`. Activation is kernel dirswap plus verify-only `claude plugin list --json`. Prepared identity walks `skills/` but does not treat hostile neighbors as a registry: only a proven same-name `plugin.json` collides. An enabled same-name marketplace plugin occupies that logical name; Claude will not load the skills-dir copy, and identity treats that occupancy as unmanaged.
...
- Repair restages and verifies Claude with list-only activation. Codex repair with a trusted CLI re-runs Activate (`plugin add`) so cache is recopied.
...
- Path identity for Claude list/`ActivePath` uses filesystem sameness so macOS `/tmp` vs `/private/tmp` and `/var` vs `/private/var` match.
