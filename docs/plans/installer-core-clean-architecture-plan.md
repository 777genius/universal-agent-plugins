...
- **1.4** — `go.work` содержит **5** модулей (`.`, `./cli`, `./install/integrationctl`, `./install/plugininstall`, `./sdk`), не 3. Lint-матрица остаётся из 3 модулей (там весь код ядра), `go vet` — по 5.
...
| 6 | Go-версии | Подтверждено + уточнение | root `go 1.22`, `install/integrationctl` `go 1.25.0`, `cli` `go 1.25.8`, toolchain везде `go1.25.13`. `go.work` (`go 1.25.8`) содержит **5** модулей: `.`, `./cli`, `./install/integrationctl`, `./install/plugininstall`, `./sdk`; root `go.mod` без `require`. `sdk` и `install/plugininstall` не импортируют `agentplugins`. Lint-матрица — 3 модуля (там весь код ядра), `go vet` в `core-fast` — по всем 5. |
...
- Поведение CLI (`--format json` и текст) неизменно (golden). **Названное исключение (Part 4):** три текста Go-ошибок, перенесённых в `clients/{claude,cursor,kiro}/plan.go`, получили ведущий артикль (`the Claude Code config root is unavailable`, `the Cursor config root is unavailable`, `a Kiro preparation requires …`). Причина — ST1005 на новых строках; форма скопирована с `providers/claude_probe.go`, который уже так пишет. Это Go-ошибки инфраструктурных предусловий, не `UserActions`/`Warnings`, они не входят ни в один golden и ни в один assert. Тот же приём применяется к 8 аналогичным строкам `providers/*_native.go` при их переносе в Part 5.
...
| `planner/compatibility.go` (ChatGPT/Windsurf limitations) | `CompatibilityLimiter` у chatgpt/windsurf; `Compatibility()` спрашивает registry. **Уточнено по факту Part 4:** имя фасада сохраняется, но сигнатура меняется — реестр приходит первым аргументом. Это package-level функция, значения планировщика у неё нет, а дефолтный реестр внутри — ровно тот service locator, который запрещает правка A-3. То же для `ApplyInstallIntent`. |
...
Приёмка: golden detection совпадает; бюджет archtest `detector` → 0; `detector.go` выходит из baseline; parity-тест в `clients/all`: каждый `domain.ClientDefinitions()` ID имеет ровно один адаптер и наоборот; `contracttest.RunHostDetector` (уникальные ID surfaces, детерминированный порядок и число probe-вызовов на повторных прогонах, на «пустом» host ничего не задетектилось — необходимое, но не достаточное условие «адаптер смотрит только через `clients.Host`»); compile-time assertions присутствуют во всех 11 пакетах.
Надёжность 9/10, уверенность 9/10.

**Решение по общей логике Copilot/VSCode (принято в Part 3, обязательно для Parts 5/7/8).** `clients/copilot` и `clients/vscode` — отдельные пакеты, а `clients-no-concrete-clients` запрещает им импортировать друг друга. При этом VERIFIED на HEAD Part 3: в generic-коде 14 упоминаний `domain.ClientVSCode` вне тестов, и почти все — общие ветки `case domain.ClientCopilot, domain.ClientVSCode` (`providers/activator.go:280,287,525,528,1058`, `providers/stager.go:242`, `providers/native_identity.go:85,222`, `providers/activation_verifier.go:23`, `planner/planner.go:39,44,233,258,342`), то есть общее поведение одного backend-семейства `github-copilot`. **Единственный легальный дом для этой общей логики — `clients/shared`** (экспортируемые хелперы, которые оба адаптера вызывают), плюс `domain.BackendSiblings`/`SharesBackend` там, где нужен «сосед по backend». Прямая зависимость `clients/vscode → clients/copilot` (в любую сторону) запрещена; копипаста между двумя пакетами тоже не вариант — она вернёт дубли, которые Part 2 только что вычистил. Конкретно: парсер вывода `copilot`-CLI, сборка Copilot-marketplace-проекции и Copilot-registry-инспекция уезжают в `clients/shared`, а `clients/copilot` и `clients/vscode` остаются тонкими обёртками над ними с разницей в `PackageMode`/`prepared`-ветке. Если в Part 5 окажется, что общего кода больше, чем различий, допустимая альтернатива — один общий тип в `clients/shared`, который оба пакета встраивают, переопределяя только `ID()`; но и тогда два пакета сохраняются, чтобы `clients/all` и archtest-allowlist оставались «один каталог = один клиент».
...
Приёмка: golden planner совпадает; бюджеты archtest `planner`, `compatibility`, `planner/intent` → 0; ветка `directory_context7_preparation.go` удалена (проверяется чтением кода — метрика archtest её не считает: внутри самого `domain` ссылки на константы неквалифицированы, а метрика считает селекторы `domain.Client<Name>`, поэтому у пакета `domain` бюджета нет и не было); `planner.go` выходит из baseline; `contracttest.RunPlanRefiner` (refiner не меняет `ClientID/Scope/PhysicalArtifactID/TargetRoot/ActivePath`, не ставит `PlanReady` поверх `PlanUnsupported`, идемпотентен); `vet-all` зелёный.
...
Изменения: `native_identity.go` оставляет generic-оркестрацию (`observeIdentity`: timeout, объединение prepared/native findings, digest verify через `Stager.Verify`, сборка `NativeIdentityObservation`); `inspectNativeRegistry` → `As[RegistryInspector]`; `nativeAttempted` = `inspector.UsesNativeRegistryExecutable() && NativeRegistryExecutable != ""`; prepared → `As[PreparedRegistryInspector]`, иначе `shared.InspectUnqualifiedPluginRoot`; `inspectCodexCLI/inspectCodexFiles/inspectCodexCache` → `clients/codex/identity.go`; `inspectCopilotCLI/copilotRegistryFindingAt` → `clients/shared` (их же зовёт `clients/vscode`: `native_identity.go:85,222` — общая ветка Copilot/VSCode, см. §8.3); `inspectKiroRegistry` → `clients/kiro`; `inspectGeminiRegistry`, `inspectWindsurfRegistry` — уже в своих пакетах. Включается depguard `generic-no-concrete` (providers/planner/clientdetect → deny `clients/<id>`; половина про `clients/all` уже включена в Part 3 как `dispatchers-take-an-injected-registry` — её надо расширить, а не дублировать; allow `clients`, `clients/shared`): после этого прямых импортов не остаётся, и тезис «сборка с подмножеством клиентов возможна» становится проверяемым, а не декларативным.
...
| **`vet-all`** (новый, правка C-1) | `go vet ./...` в каждом модуле `go.work`: `.`, `cli`, `install/integrationctl`, `install/integrationctl/agentplugins`, `install/plugininstall`, `sdk` | 1-2 мин |
...
13. **Lint-гейт гоняется только на ubuntu (`GOOS=linux`) — ПРИНЯТЫЙ остаточный риск.** Файлы за build-тегами `windows`/`darwin` (`*_windows.go`, `*_darwin.go`) линтер в CI не видит вообще, поэтому size/style-гейт на них не действует и они могут расти без ограничений. Корректность компиляции по трём GOOS всё ещё проверяется job'ом `cross-build` в `core-fast` (§10). Компромисс в пользу скорости CI: матрица `lint.yml` по трём GOOS дала бы 9 job вместо 3 ради style/size-проверки легаси-файлов. Обнаружено в Part 0b: LEGACY SIZE BASELINE был сгенерирован на macOS и не содержал ни одного платформенного файла, из-за чего `lint / install/integrationctl` покраснел на `adapters/process/osrunner_tree_linux.go`; исправлено отдельным fix-forward PR (6 записей, сгенерированных прогонами `GOOS=linux` и `GOOS=windows`, + требование к регенерации в комментарии `.golangci.yml`). Если понадобится закрыть риск полностью — расширить `lint.yml` до матрицы по GOOS отдельной частью.
