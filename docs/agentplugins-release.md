...
`agentplugins-v0.1.64` and `plugin-kit-ai-v2.0.4`. Merge the consumer first, then
select the **final merged source SHA** for both tags and all retained release
qualification. Dispatch this workflow at `refs/tags/agentplugins-v0.1.64` with
`tag=agentplugins-v0.1.64`, `plugin_kit_version=2.0.4`, and that `source_sha`.
...
The job constructs `universal-agent-plugins@0.1.64` and `plugin-kit-ai@2.0.4`
...
2 and later. The Python wrapper accepts numeric 2.0.4 or
`plugin-kit-ai-v2.0.4`, extracts numeric asset versions, and preserves historical
