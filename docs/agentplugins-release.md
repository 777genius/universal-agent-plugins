...
- This workflow has one agentplugins-only dispatch contract. It qualifies a
...
1. Create the approved annotated stable tag on current `main` and push only that
   tag.
2. Dispatch `Agentplugins Release Assets` with the exact tag. Leave the typed boolean `publish_release=false`
...
## Retired publication surface

Standalone `plugin-kit-ai` npm and PyPI publishing and all paired release modes are retired. Do not recreate or dispatch those paths, and do not add
`agentplugins` to PyPI. Existing package source, producer scripts, tests,
fixtures, and historical release evidence remain preserved as implementation
history; their presence does not make them a supported publication channel.
