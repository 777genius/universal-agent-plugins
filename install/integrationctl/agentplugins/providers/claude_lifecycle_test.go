...
func TestClaudeProbeEnvironmentRejectsDuplicateAllowedVariableAndDropsOverrides(t *testing.T) {
	home := t.TempDir()
	environment, gotHome, err := claude.BoundedProbeEnvironmentFrom([]string{
		"HOME=" + home,
		"PATH=/usr/bin:/bin",
	...
	if _, _, err := claude.BoundedProbeEnvironmentFrom([]string{"HOME=" + home, "PATH=/bin", "PATH=/attacker"}); err == nil {
...
func TestClaudeProbeUsesFullDescendantContainmentWithFiveSecondGrace(t *testing.T) {
	...
	result, err := claude.RunListCommand(context.Background(), runner, command)
...
`
	...
	command, err := claude.ListCommand(script, config, active)
	if err != nil {
		t.Fatal(err)
	}
