...
func TestOpenCodeNativeAddUpdateVerifyRemovePreservesJSONCAndForeignConfig(t *testing.T) {
	...
	firstObjects, err := BuildNativeObjects(active, firstEnvelope, firstPlan)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := VerifyNativeObjects(configRoot, active, firstObjects); err != nil {
		t.Fatal(err)
	}
	...
	if err := ProjectNative(active, secondEnvelope, secondPlan, filepath.Join(root, "data")); err != nil {
		t.Fatal(err)
	}
	secondObjects, err := BuildNativeObjects(active, secondEnvelope, secondPlan)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenCodeLateSkillCollisionDoesNotReplaceForeignDirectory(t *testing.T) {
	...
	err = applyOpenCodeNativeMutation(clients.Env{NativeConfig: nativeconfig.New()}, clients.Ops{Rename: rename}, configRoot, active, nil, objects)
...
func TestOpenCodeRollbackDoesNotReplaceConcurrentForeignDirectory(t *testing.T) {
	...
	first, err := BuildNativeObjects(activeV1, firstEnvelope, firstPlan)
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := BuildNativeObjects(activeV2, secondEnvelope, secondPlan)
	if err != nil {
		t.Fatal(err)
	}
	liveSkill := filepath.Join(configRoot, "skills", "docs")
	...
	err = applyOpenCodeNativeMutation(clients.Env{NativeConfig: nativeconfig.New()}, clients.Ops{Rename: rename}, configRoot, activeV2, first, second)
...
func TestOpenCodeCommittedCleanupFailureKeepsReceiptsAndLaterLifecycleConsistent(t *testing.T) {
	...
	cleanupCalls := 0
	err = applyOpenCodeNativeMutation(clients.Env{NativeConfig: nativeconfig.New()}, clients.Ops{Rename: shared.RenameDirectoryExclusive, RemoveAll: func(path string) error {
		cleanupCalls++
		if !strings.HasPrefix(filepath.Base(path), ".agentplugins-native-") {
			t.Fatalf("cleanup target = %s", path)
		}
		return cleanupErr
	}}, configRoot, activeV2, first, second)
	if err != nil {
	...
	if err := VerifyNativeObjects(configRoot, activeV2, second); err != nil {
...
func TestOpenCodeFailedUpdateRestoresPreviousManagedSkill(t *testing.T) {
	...
	active := filepath.Join(root, "managed", "demo")
	firstEnvelope, firstPlan := openCodeTestPackage(t, active, configRoot, "old")
	first, err := BuildNativeObjects(active, firstEnvelope, firstPlan)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondEnvelope, secondPlan := openCodeTestPackage(t, active, configRoot, "new")
	second, err := BuildNativeObjects(active, secondEnvelope, secondPlan)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenCodeProjectsClientManagedStdioDataContract(t *testing.T) {
	...
	if err := ProjectNative(active, envelope, plan, dataRoot); err != nil {
		t.Fatal(err)
	}
	objects, err := BuildNativeObjects(active, envelope, plan)
	if err != nil {
...
func TestOpenCodeRejectsReservedStdioEnvWithoutProjection(t *testing.T) {
	...
	err := ProjectNative(active, envelope, plan, filepath.Join(root, "data"))
...
func TestOpenCodeAmbiguousJSONVariantsFailBeforeProjection(t *testing.T) {
	...
	if err := ProjectNative(active, envelope, plan, ""); !errors.Is(err, nativeconfig.ErrAmbiguousConfig) {
...
func TestOpenCodeRejectsPackageOwnedProjectionCollision(t *testing.T) {
	...
	plan := domain.DeliveryPlan{ClientID: domain.ClientOpenCode, NativeRegistryRoot: filepath.Join(root, "config"), ActivePath: active}
	err := ProjectNative(active, domain.PackageEnvelope{}, plan, "")
...
func openCodeTestPackage(t *testing.T, active, configRoot, skillText string) (domain.PackageEnvelope, domain.DeliveryPlan) {
	...
	if err := ProjectNative(active, envelope, plan, filepath.Join(filepath.Dir(active), "data")); err != nil {
		t.Fatal(err)
	}
