...
const (
	RuntimeDirectory       = ".agentplugins-runtime"
	claudeRuntimeDirectory = RuntimeDirectory
	claudeRuntimeSource    = ".agentplugins-runtime-source"
)

// Project renders the portable envelope into Claude Code's official
...
func Project(root string, envelope domain.PackageEnvelope, plan domain.DeliveryPlan, dataPath string) error {
	runtimeRoot, err := IsolateRuntime(root)
	if err != nil {
		return err
	}
	if err := pruneRuntimeSkills(runtimeRoot, plan); err != nil {
		return err
	}
	...
	if err := ProjectMCP(root, envelope, serverNames, activeRuntimeRoot, dataPath, runtimeRoot); err != nil {
		return err
	}
	return ValidateProjectionRoot(root, len(serverNames) > 0)
}

func IsolateRuntime(root string) (string, error) {
...
func pruneRuntimeSkills(runtimeRoot string, plan domain.DeliveryPlan) error {
...
func ValidateProjectionRoot(root string, wantMCP bool) error {
...
func ProjectMCP(root string, envelope domain.PackageEnvelope, serverNames []string, pluginRoot, dataPath string, observationRoot ...string) error {
