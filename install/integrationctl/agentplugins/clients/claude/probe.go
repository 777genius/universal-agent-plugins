...
const (
	...
	claudeProbeTimeout             = ProbeTimeout
)
...
func PrepareActivationProbe(request domain.ActivationRequest) (ActivationProbe, error) {
	configRoot := filepath.Clean(strings.TrimSpace(request.Client.ConfigRoot))
	if !filepath.IsAbs(configRoot) {
		return ActivationProbe{}, fmt.Errorf("absolute Claude Code config root is required")
	}
	...
	if err != nil {
		return ActivationProbe{}, err
	}
	return ActivationProbe{Command: command, ConfigRoot: configRoot, ActivePath: activePath}, nil
}
...
func BoundedProbeEnvironment() ([]string, string, error) {
	return BoundedProbeEnvironmentFrom(os.Environ())
}

func BoundedProbeEnvironmentFrom(ambient []string) ([]string, string, error) {
	values := map[string]string{}
	for _, entry := range ambient {
		name, value, ok := strings.Cut(entry, "=")
		if !ok || name == "" || !allowedProbeEnvironment(name) {
			continue
		}
...
func allowedProbeEnvironment(name string) bool {
