...
// ManagedMarketplaceRegistered proves that the exact manager-owned
// marketplace still points at the exact managed package before native cleanup.
// A same-name user replacement is never removed.
func ManagedMarketplaceRegistered(configRoot, marketplace, managedArtifactPath string) (bool, error) {
...
// ManagedPluginEntryPresent reports whether Codex's config.toml still
...
// signal here (unlike ManagedMarketplaceRegistered's source-path check):
...
func ManagedPluginEntryPresent(configRoot, declaredName, marketplace string) (bool, error) {
