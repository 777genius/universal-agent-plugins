...
var (
	ErrACPContractUnknown = errors.New("Kiro structured ACP verification is unavailable or unrecognized")
...
func verifyACPWithTimeout(ctx context.Context, runner ports.DuplexCommandRunner, executable, cwd string, servers []string, startupTimeout time.Duration) error {
	...
	for _, rawName := range servers {
		...
		if name == "" {
			return fmt.Errorf("%w: planned MCP server name is empty", ErrACPContractUnknown)
		}
		if _, duplicate := expected[name]; duplicate {
			return fmt.Errorf("%w: duplicate planned MCP server identity %q", ErrACPContractUnknown, name)
		}
		expected[name] = &kiroACPServerState{}
	}
	if len(expected) == 0 {
		return fmt.Errorf("%w: no planned MCP servers", ErrACPContractUnknown)
	}
	...
	return fmt.Errorf("%w: %v", ErrACPContractUnknown, err)
}
