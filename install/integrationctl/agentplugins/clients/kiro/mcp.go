...
func ReadMCPConfig(path string) (servers map[string]any, original []byte, mode os.FileMode, exists bool, err error) {
	...
	if statErr != nil || info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
		return nil, nil, mode, false, fmt.Errorf("the Kiro MCP configuration must be a regular file")
	}
	...
	if raw, present := document["mcpServers"]; present {
		...
		if !exists {
			return nil, nil, mode, false, fmt.Errorf("the Kiro mcpServers must be an object")
		}
	} else {
