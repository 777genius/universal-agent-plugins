...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	historicalProse := strings.Join(strings.Fields(historicalReadme), " ")
	mustContain(t, currentProse, "root `plugin.json` with optional `skills/` and `mcp.json`")
	mustContain(t, currentProse, "`agentplugins author` is the sole public entrypoint to the shared standard authoring engine.")
	...
	mustContain(t, currentProse, "Milestone A static authoring is available. Runtime, preview/export and publication phases remain deferred.")
	...
	mustContain(t, currentProse, "Legacy YAML migration is cancelled because there are no known users")
	mustContain(t, historicalReadme, "[Historical plugin-kit-ai v1, baseline 1.2.4](website/source/en/legacy/v1/index.md)")
	mustContain(t, historicalProse, "The legacy product is retired and no migration journey is planned.")
	...
	mustContain(t, historicalReadme, "The exact historical source remains available from the pinned archive above.")
	...
	mustContain(t, historicalReadme, "[local examples](examples/local/README.md)")
