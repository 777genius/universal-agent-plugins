...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, currentProse, "`agentplugins author` is the public authoring entrypoint.")
	mustContain(t, currentProse, "npm install -g universal-agent-plugins")
	mustContain(t, currentProse, "agentplugins-v*")
	mustNotContain(t, currentProse, "npm install -g universal-agent-plugins@")
	mustNotContain(t, currentProse, "plugin-kit-ai")
	mustNotContain(t, currentProse, "Milestone A")
	mustNotContain(t, currentProse, "Historical authoring and development")
	mustContain(t, currentProse, "MCP execution is available through `author test` and `author dev`.")
	mustContain(t, currentProse, "Dependency bootstrap, client projection, export/bundle, and publication are not exposed by the current authoring CLI.")
	mustContain(t, currentProse, "`plugin.json` is the only supported authoring manifest.")
	...
	mustContain(t, string(releaseDoc), "manual-only publishers for")
	mustContain(t, string(releaseDoc), "runtime helpers")
	mustContain(t, string(releaseChecklist), "intentionally retained `plugin-kit-ai-runtime` helper changed")
	mustContain(t, string(releaseChecklist), "do not present a runtime helper as a CLI installation channel")
	...
	mustContain(t, string(releaseDoc), "`agentplugins` is the sole public CLI")
	mustContain(t, string(releaseDoc), "No new `plugin-kit-ai` CLI GitHub, npm, PyPI, Homebrew, or native release is supported")
	mustContain(t, string(releaseDoc), "docs/history/plugin-kit-ai-release-workflows")
	mustNotContain(t, string(releaseDoc), "pipx install plugin-kit-ai")
	...
	mustContain(t, string(releaseChecklist), "dispatch `agentplugins-release.yml`")
	mustContain(t, string(releaseChecklist), "protected `npm-agentplugins`")
	mustContain(t, string(releaseChecklist), "no executable workflow under `.github/workflows` publishes or preflights")
	mustContain(t, string(releaseNotesTemplate), "- agentplugins draft qualification:")
	mustContain(t, string(releaseNotesTemplate), "- universal-agent-plugins npm publish and public verification:")
	mustContain(t, string(rehearsalTemplate), "sole public CLI remains `agentplugins`")
	mustContain(t, string(rehearsalTemplate), "agentplugins native release and attestations")
	...
	homebrewTapWorkflow, err := os.ReadFile(filepath.Join(root, "docs", "history", "plugin-kit-ai-release-workflows", "homebrew-tap.yml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	npmPublishWorkflow, err := os.ReadFile(filepath.Join(root, "docs", "history", "plugin-kit-ai-release-workflows", "npm-publish.yml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	pypiPublishWorkflow, err := os.ReadFile(filepath.Join(root, "docs", "history", "plugin-kit-ai-release-workflows", "pypi-publish.yml"))
	if err != nil {
		t.Fatal(err)
	}
