...
function env() {
  return { GITHUB_ACTIONS: "true", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REPOSITORY: c.REPOSITORY,
    PRODUCER_MODE: "paired-publish", TAG: m.TAG, KIT_VERSION: "2.0.3", SOURCE_SHA: source,
...
function fixture() {
  ...
  const record = { schema: m.RECORD_SCHEMA, identity: { repository: c.REPOSITORY, commit: source,
    engine_revision: source, versions: { agentplugins: "0.1.63", "plugin-kit-ai": "2.0.3" } },
...
for (const status of [401, 403, 429, 500, 503]) test(`registry HTTP ${status} cannot authorize publication`, async t => {
  ...
  let writes = 0;
  await assert.rejects(p.publishOnce({ lookup: () => p.registry("https://registry.npmjs.org/universal-agent-plugins/0.1.63", true),
...
test("network uncertainty cannot authorize publication", async t => {
  ...
  await assert.rejects(p.registry("https://registry.npmjs.org/universal-agent-plugins/0.1.63", true));
});
test("only completed 404 is absence", async t => {
  t.mock.method(globalThis, "fetch", async () => new Response("not found", { status: 404 }));
  assert.equal(await p.registry("https://registry.npmjs.org/universal-agent-plugins/0.1.63", true), null);
  await assert.rejects(p.registry("https://registry.npmjs.org/universal-agent-plugins/0.1.63"));
});
...
test("exact paired source contract rejects synthetic versions and different promotion", () => {
  const f = fixture(), metadata = { name: "universal-agent-plugins", version: "0.1.63", gitHead: source };
...
test("qualified public package packs once with exact closure and no GitHub notices asset", sourceOnly, t => {
  ...
  assert.equal(pkg.version, "0.1.63"); assert.equal(pkg.private, false);
...
test("unsigned or cryptographically invalid npm audit cannot reach authoring smoke", async t => {
  ...
  t.mock.method(cp, "execFileSync", (exe, args, options) => {
    ...
    if (args.includes("install")) { assert.ok(args.includes("--ignore-scripts")); assert.ok(args.includes("universal-agent-plugins@0.1.63")); return Buffer.from(""); }
...
test("verified readback performs only disposable exact-version smoke, with publisher credentials removed", async t => {
  ...
  t.mock.method(cp, "execFileSync", (exe, args, options) => {
    ...
    if (args.includes("install")) {
      assert.equal(args.at(-1), "universal-agent-plugins@0.1.63");
      ...
      return Buffer.from(JSON.stringify({ invalid: [], missing: [], verified: [{ name: "universal-agent-plugins",
        version: "0.1.63", location: "node_modules/universal-agent-plugins", registry: "https://registry.npmjs.org/",
...
test("prepublication smoke uses exact tarball, actual lifecycle, disposable roots and no publisher credentials", t => {
  ...
  t.mock.method(cp, "execFileSync", (exe, args, options) => {
    ...
    assert.deepEqual(project.allowScripts, { "file:/exact/plugin-kit-ai-2.0.3.tgz": true });
  ...
  p.packedSmoke("plugin-kit-ai", "/exact/plugin-kit-ai-2.0.3.tgz", dir, "/fixture/npm.js", context);
  assert.ok(calls[0].includes("--ignore-scripts=false")); assert.equal(calls[0].at(-1), "/exact/plugin-kit-ai-2.0.3.tgz");
...
test("kit readback verifies product-specific registry, signatures, postinstall and launcher", async t => {
  ...
  t.mock.method(cp, "execFileSync", (exe, args, options) => {
    ...
    if (args.includes("install")) {
      assert.equal(args.at(-1), "plugin-kit-ai@2.0.3"); assert.ok(args.includes("--ignore-scripts"));
      ...
      name: product, version: "2.0.3", location: `node_modules/${product}`, registry: "https://registry.npmjs.org/",
      attestations: f.metadata.dist.attestations, attestationBundles: f.response.attestations }] }));
    return Buffer.from("");
  });
...
test("legacy copy-only publisher refuses v2 before authentication or package staging", sourceOnly, t => {
  ...
  for (const tag of ["1.2.4", "v1.2.4", "plugin-kit-ai-v1.2.4", "2.0.2", "v2.0.2", "plugin-kit-ai-v2.0.2",
    "2.0.3", "v2.0.3", "plugin-kit-ai-v2.0.3", "agentplugins-v0.1.63"]) {
