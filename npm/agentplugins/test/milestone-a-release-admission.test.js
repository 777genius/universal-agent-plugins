...
test("current release tags are the exact prefixed patch pair", () => {
  assert.equal(a.TAG, "agentplugins-v0.1.63");
  assert.equal(a.KIT_TAG, "plugin-kit-ai-v2.0.3");
});
...
test("binds behavioral receipts to the source and synthetic 0.1.91 test package without claiming release 0.1.63", () => {
  ...
  assert.deepEqual(result.release_versions, { agentplugins: "0.1.63", "plugin-kit-ai": "2.0.3" });
...
for (const [name, mutate] of [
  ["synthetic package presented as 0.1.63", value => { value.prepare.identity.versions.agentplugins = "0.1.63"; }],
