...
test("actual controlled Linux pair: offline verification, engine reports, frozen journeys and negative proof", {
  ...
  const sharedReports = reports.map((productReports, productIndex) => productReports.map(({ argv, report }) => {
    ...
    if (normalized.commands) {
      for (const name of ["author.bootstrap", "author.dev", "author.import.native", "author.normalize"])
        assert.equal(normalized.commands.includes(name), productIndex === 0);
      normalized.commands = normalized.commands.filter((name) =>
        !["author.bootstrap", "author.dev", "author.import.native", "author.normalize"].includes(name));
    }
    if (argv.includes("capabilities")) {
      ...
      normalized.capabilities.commands = commands.filter((name) =>
        !["author.bootstrap", "author.dev", "author.import.native", "author.normalize"].includes(name));
