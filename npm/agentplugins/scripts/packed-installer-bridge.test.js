...
function publicFixture(t) {
 ...
 for(const p of bridge.PRODUCTS) {
  ...
  write(path.join(outputs[p],'release-manifest.json'),{schema_version:3,status:'CANDIDATE',product:p,repository:old.identity.repository,tag:p==='agentplugins'?`agentplugins-v${old.identity.versions[p]}`:`plugin-kit-ai-v${old.identity.versions[p]}`,version:old.identity.versions[p],versions:old.identity.versions,authoring_mode:old.authoringMode,asset_scope:'six-platform-pair',assets:assets[p].assets,commit:old.identity.commit,engine_revision:old.identity.commit,candidate_sha256:old.manifestDigest,release_eligible:false,platform_acceptance:false,attested:false});
