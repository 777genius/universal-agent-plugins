...
function selection(e) {
  ...
  assert.equal(e.KIT_VERSION, "2.0.1");
...
function packageFiles(source, manifest, record) {
  milestone.recordShape(record);
  const files = require("./stage-authoring-npm").packageFiles("agentplugins", source, manifest,
  ...
  assert.equal(c.digest(manifest), record.products.agentplugins.manifest_sha256);
  files["public-release.json"] = c.encode(descriptor);
  const pkg = JSON.parse(files["package.json"]);
  delete pkg.private;
  pkg.gitHead = record.identity.commit;
...
function prepare(selected, repo, scratch, output, npm) {
  const state = admit(selected, repo, scratch);
  const context = packing.npmContext(scratch);
  const source = packing.blobs(repo, selected.source, context.env, "public");
  const manifest = c.readFile(path.join(state.o.root, "agentplugins", "release-manifest.json"));
  const files = packageFiles(source, manifest, state.record);
  ...
  fs.mkdirSync(output);
  packing.packPackage("agentplugins", files, root, { node: process.execPath, npm, output,
...
function receiptBytes(root, selected) {
...
async function reconcile(receipt, body, scratch, npm, env) {
  const metadata = JSON.parse(await registry(`${REGISTRY}/${NAME}/${VERSION}`));
  contract.validatePublicMetadata(metadata, VERSION, receipt.integrity, receipt.shasum);
  assert.equal(metadata.gitHead, receipt.source, "npm source binding");
  ...
  contract.validateSLSAAttestation(response, VERSION, receipt.integrity, milestone.TAG, receipt.source);
  ...
  try {
    ...
    contract.validateAuditSignatures(audit, response, VERSION);
    ...
    contract.validatePairedSource(metadata, descriptor, receipt.source, receipt.promotion_sha256);
    // Only exact-version authoring in this new disposable project; no installer
    // registry repair, user project, or broad E2E replay.
    const cli = path.join(installed, "bin/agentplugins.js");
    run(process.execPath, [cli, "author", "--help"], project, env);
    run(process.execPath, [cli, "author", "init", "smoke", "--template=skill", "--name=paired-smoke", "--description=Disposable publication smoke.", "--format=json"], project, env);
    run(process.execPath, [cli, "author", "validate", "smoke"], project, env);
...
async function publish(selected, repo, scratch, root, npm) {
  ...
  checkout(repo, selected.source);
  const { receipt, body } = receiptBytes(root, selected);
  ...
  const context = packing.npmContext(scratch);
  const source = packing.blobs(repo, selected.source, context.env, "public");
  const files = packageFiles(source, c.readFile(path.join(state.o.root, "agentplugins", "release-manifest.json")), state.record);
  ...
  return publishOnce({ lookup: () => registry(`${REGISTRY}/${NAME}/${VERSION}`, true),
    ...
    }, reconcile: () => reconcile(receipt, body, scratch, npm, env) });
}
async function main(args) {
  assert.equal(args.length, 3);
  const [mode, root, npm] = args;
  ...
  return mode === "prepare" ? prepare(selected, repo, scratch, root, npm) : publish(selected, repo, scratch, root, npm);
}
module.exports = { admit, reconcile, selection, publicPair, pairIdentity, promotionInvocation, packageFiles, byteIdentity, registry, receiptBytes, publishOnce };
